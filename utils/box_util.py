...
# let's say width is 576. for neural motifs it was 576*576 pixels so 331776.
# here we have 2x*x = 331776-> 408 base
...
if USE_IMAGENET_PRETRAINED:
    def to_tensor_and_normalize(image):
        return functional.normalize(functional.to_tensor(image),
    ...
    def to_tensor_and_normalize(image):
        ...
        return functional.normalize(tensor255,
                                    mean=(102.9801, 115.9465, 122.7717),
                                    std=(1, 1, 1))


def img2_tensor_and_normalize(image):
    return functional.normalize(functional.to_tensor(image),
                                mean=(0.485, 0.456, 0.406),
                                std=(0.229, 0.224, 0.225))
