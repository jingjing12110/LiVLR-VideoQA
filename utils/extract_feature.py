...
# @Github : https://github.com/AmingWu/CCN
...
class SimpleDetector(nn.Module):
    def __init__(self, pretrained=True, average_pool=True, semantic=True,
        ...
        :param semantic: Whether or not we want to introduce the mask and the
        class label early on (default Yes)
        """
        super(SimpleDetector, self).__init__()
        backbone = _load_resnet_imagenet(
            pretrained=pretrained) if USE_IMAGENET_PRETRAINED else _load_resnet(
        ...
        if semantic:
            ...
            self.mask_upsample = torch.nn.Conv2d(1, self.mask_dims,
                                                 kernel_size=3,
                                                 stride=2 if
                                                 USE_IMAGENET_PRETRAINED else 1,
                                                 padding=1,
                                                 bias=True)
        else:
    ...
    def forward(self,
        ...
        :param box_mask: [batch_size, max_num_objects] Mask for
            whether or not each box is OK
        :param classes:
        :param segms:
        ...
        # Add some regularization, encouraging the model to keep giving decent
        # enough predictions
        obj_logits = self.regularizing_predictor(post_roialign)
        obj_labels = classes[box_inds[:, 0], box_inds[:, 1]]
        cnn_regularization = \
            F.cross_entropy(obj_logits, obj_labels, size_average=True)[None]

        feats_to_downsample = post_roialign if self.object_embed is None \
            else torch.cat((post_roialign, self.object_embed(obj_labels)), -1)
        roi_aligned_feats = self.obj_downsample(feats_to_downsample)

        # Reshape into a padded sequence - this is expensive and annoying but
        # easier to implement and debug...
        obj_reps = pad_sequence(roi_aligned_feats, box_mask.sum(1).tolist())
        return {'obj_reps_raw': post_roialign,
                'obj_reps': obj_reps,
...
class ImageFeatureExtractor(nn.Module):
    def __init__(self):
        ...
        self.resnet_101.layer4.register_forward_hook(save_output)

    def forward(self, imgs):
        self.resnet_101(imgs)
