...
class ContrastiveLoss(nn.Module):
    ...
    def forward(self, scores, margin=None, average_batch=True):
        """the same row of im and s are positive pairs, different rows are
            negative pairs
        @param scores: image-sentence score matrix, (batch, batch)
        @param margin:
        @param average_batch:
        @return:
        """
        ...
        if self.direction == 'i2t' or self.direction == 'bi':
            d1 = diagonal.expand_as(
                scores)  # same collumn for im2s (negative sentence)
        ...
        if self.direction == 't2i' or self.direction == 'bi':
            d2 = diagonal.t().expand_as(
                scores)  # same row for s2im (negative image)
