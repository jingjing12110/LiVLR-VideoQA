...
def extra_leading_dim_in_sequence(f, x, mask):
    return f(x.view(-1, *x.shape[2:]), mask.view(-1, mask.shape[2])).view(
        *x.shape[:3], -1)
...
def clip_grad_norm(named_parameters, max_norm, clip=True, verbose=False):
    ...
    if verbose:
        print('---Total norm {:.3f} clip coef {:.3f}-----------------'.format(
            total_norm, clip_coef))
        for name, norm in sorted(param_to_norm.items(), key=lambda x: -x[1]):
            print("{:<60s}: {:.3f}, ({}: {})".format(name, norm, np.prod(
                param_to_shape[name]), param_to_shape[name]))
        print('-------------------------------', flush=True)
...
def find_latest_checkpoint(serialization_dir):
    ...
    have_checkpoint = (serialization_dir is not None and
                       any("model_state_epoch_" in x for x in
                           os.listdir(serialization_dir)))

    if not have_checkpoint:
        return None

    serialization_files = os.listdir(serialization_dir)
    model_checkpoints = [x for x in serialization_files if
                         "model_state_epoch" in x]
    ...
    training_state_path = os.path.join(serialization_dir,
                                       "training_state_epoch_{}.th".format(
                                           epoch_to_load))
    return model_path, training_state_path


def save_checkpoint(model, optimizer, serialization_dir, epoch,
                    val_metric_per_epoch, is_best=None,
    ...
    if serialization_dir is not None:
        ...
        model_state = model.module.state_dict() if isinstance(model,
                                                              DataParallel) \
        ...
        if is_best:
            print(f"Best validation performance so far. "
                  f"Copying weights to '{serialization_dir}/best.th'.")
...
def restore_checkpoint(model, optimizer, serialization_dir,
                       learning_rate_scheduler=None):
    """
    Restores a model from a serialization_dir to the last saved checkpoint.
    This includes an epoch count and optimizer state,
    which is serialized separately
    from  model parameters. This function should only be used to continue
    training - if you wish to load a model for inference/load parts of a model
    into a new computation graph, you should use the native Pytorch functions:
    `` model.load_state_dict(torch.load("/path/to/model/weights.th"))``
    If ``self._serialization_dir`` does not exist or does not contain any
    checkpointed weights, this function will do nothing and return 0.
    Returns
    -------
    epoch: int
...
def detokenize(array, vocab):
    ...
    return ' '.join([x for x in tokenized if
...
def print_para(model):
    ...
    for p_name, p in model.named_parameters():
        st[p_name] = (
            [str(x) for x in p.size()], np.prod(p.size()), p.requires_grad)
    ...
    shapes_df = pd.DataFrame(
        [(p_name, '[{}]'.format(','.join(size)), prod, p_req_grad)
         for p_name, (size, prod, p_req_grad) in
         sorted(st.items(), key=lambda x: -x[1][1])],
        columns=['name', 'shape', 'size', 'requires_grad']).set_index('name')

    # print(f'\n {total_params / 1000000.0:.1f}M total parameters. '
    #       f'{total_params_training / 1000000.0:.1f}M training \n '
    #       f'\n {shapes_df.to_string()} \n')
...
def batch_iterator(seq, batch_size, skip_end=True):
    for b_start, b_end in batch_index_iterator(len(seq), batch_size,
                                               skip_end=skip_end):
