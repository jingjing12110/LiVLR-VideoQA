...
def pad_sequences(sequences, pad_token="[PAD]", pad_location="LEFT",
                  max_length=None):
    """
    Pads all sequences to the same length.
    The length is defined by the longest sequence.
...
def convert_sent_to_index(sentence, word_to_index):
    ...
    """
    return [word_to_index[word] if word in word_to_index.keys() else 0 for word
            in sentence]
    ...
    for batch_num in range(num_batches):
        ...
        if fill is True and end_index >= data_length:
            ...
            selected_indices = np.concatenate(
                [selected_indices, random.randint(0, data_length, num_missing)])
        yield [data[i] for i in selected_indices]


def fsr_iter(fsr_data, batch_size, random_seed=42, fill=True):
    """
    fsr_data : one of LSMDCData.build_data(),
    [[video_features], [sentences], [roots]]
    return per iter :
    [[feature]*batch_size, [sentences]*batch_size, [roots]*batch]
    ...
    train_iter = batch_iter(list(zip(*fsr_data)), batch_size, fill=fill,
                            seed=random_seed)
...
def preprocess_sents(descriptions, word_to_index, max_length):
    ...
    descriptions = [convert_sent_to_index(sent, word_to_index) for sent in
                    descriptions]
...
def preprocess_roots(roots, word_to_index):
    ...
    roots = [word_to_index[root] if root in word_to_index.keys() else 0 for root
             in roots]
...
def pad_video(video_feature, dimension):
    """Fill pad to video to have same length. Pad in Left.
    ...
    if num_padding == 0:
        ...
        steps = np.linspace(0, current_length, num=max_length, endpoint=False,
                            dtype=np.int32)
