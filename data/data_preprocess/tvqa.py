# @File : tvqa.py
# @Github : https://github.com/jayleicn/TVQA
...
from utils.tools import load_pickle, save_pickle, load_json, files_exist
...
class TVQADataset(Dataset):
    def __init__(self, opt, mode="train"):
        ...
        # set entry keys
        if self.with_ts:
            self.text_keys = ["q", "a0", "a1", "a2", "a3", "a4",
                              "located_sub_text"]
        else:
        ...
        for k in self.text_keys + [self.vcpt_key, self.qid_key,
                                   self.vid_name_key]:
            if k == "vcpt":
                continue
        ...
        if not files_exist([self.word2idx_path, self.idx2word_path,
    ...
    def build_word_vocabulary(self, word_count_threshold=0):
        ...
        vocab = [w for w in word_counts if word_counts[
            w] >= word_count_threshold and w not in self.word2idx.keys()]
        print(
        ...
        print(
            "Loading glove embedding at path : %s. \n" % self.glove_embedding_path)
...
def preprocess_inputs(batched_data, max_sub_l, max_vcpt_l, max_vid_l,
                      device="cuda:0"):
...
if __name__ == "__main__":
    # python tvqa.py --input_streams sub
    import sys
    from config.config_TVQA import BaseOptions

    sys.argv[1:] = ["--input_streams", "sub"]
    opt = BaseOptions().parse()

    dset = TVQADataset(opt, mode="valid")
    data_loader = DataLoader(dset, batch_size=10, shuffle=False,
                             collate_fn=pad_collate)

    for batch_idx, batch in enumerate(data_loader):
        model_inputs, targets, qids = preprocess_inputs(batch, opt.max_sub_l,
                                                        opt.max_vcpt_l,
                                                        opt.max_vid_l)
        break
