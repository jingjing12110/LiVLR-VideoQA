...
from utils.data_util import log
...
class DatasetTGIF:
    ...
    def __init__(self, args, mode='train', image_feature_net='resnet',
                 layer='pool5', max_length=80,
                 max_n_videos=None, data_type=None):
        self.args = args
        self.mode = mode
        self.qa_dir = self.args.data_dir + 'tgif_qa/'
        self.vocabulary_dir = self.args.data_dir + 'Vocabulary/'
        self.video_feat_dir = self.args.data_dir + 'feats/'

        self.image_feature_net = image_feature_net
        self.layer = layer
        self.max_length = max_length
        self.max_n_videos = max_n_videos
        self.data_type = data_type  # FrameQA, Trans, Count, Action

        self.type_dict = {'FrameQA': 'Train_frameqa_question.csv',
    ...
    def read_tgif_from_hdf5(self):
        ...
        if self.image_feature_net.upper() == "CONCAT":
            if self.layer.lower() == "fc":
                c3d_file = os.path.join(self.video_feat_dir,
                                        "TGIF_C3D_fc6.hdf5")
                resnet_file = os.path.join(self.video_feat_dir,
                                           "TGIF_RESNET_pool5.hdf5")
            elif self.layer.lower() == "conv":
                c3d_file = os.path.join(self.video_feat_dir,
                                        "TGIF_C3D_conv5b.hdf5")
                resnet_file = os.path.join(self.video_feat_dir,
                                           "TGIF_RESNET_res5c.hdf5")
            return {"c3d": h5py.File(c3d_file, 'r'),
                    "resnet": h5py.File(resnet_file, 'r')}
        else:
            feature_file = os.path.join(self.video_feat_dir,
                                        "TGIF_" + self.image_feature_net.upper() + "_" + self.layer.lower() + ".hdf5")
            assert_exists(feature_file)
            log.info("Load %s hdf5 file: %s", self.image_feature_net.upper(),
                     feature_file)
            return h5py.File(feature_file, 'r')

    def read_df_from_csvfile(self):
        assert self.data_type in ['FrameQA', 'Count', 'Trans',
                                  'Action'], 'Should choose data type '

        if self.data_type == 'FrameQA':
            train_data_path = os.path.join(self.qa_dir,
                                           'Train_frameqa_question.csv')
            test_data_path = os.path.join(self.qa_dir,
                                          'Test_frameqa_question.csv')
            self.total_q = pd.DataFrame().from_csv(
                os.path.join(self.qa_dir, 'Total_frameqa_question.csv'),
                sep='\t')
            ...
            self.total_q = pd.DataFrame().from_csv(
                os.path.join(self.qa_dir, 'Total_count_question.csv'), sep='\t')
            ...
            self.total_q = pd.DataFrame().from_csv(
                os.path.join(self.qa_dir, 'Total_transition_question.csv'),
                sep='\t')
            ...
            self.total_q = pd.DataFrame().from_csv(
                os.path.join(self.qa_dir, 'Total_action_question.csv'),
                sep='\t')
    ...
    def __repr__(self):
        if hasattr(self, 'word2idx'):
            return '<Dataset (%s) with %d videos and %d words>' % (
                self.mode, len(self), len(self.word2idx))
        else:
            return '<Dataset (%s) with %d videos -- dictionary not built>' % (
                self.mode, len(self))

    @staticmethod
    def split_sentence_into_words(sentence, eos=True):
        """
    ...
    def build_word_vocabulary(self, all_captions_source=None,
        ...
        log.infov('Building word vocabulary (%s) ...', self.mode)
        ...
        vocab = [w for w in word_counts if
                 word_counts[w] >= word_count_threshold]
        ...
        pkl.dump(self.word2idx, open(os.path.join(self.vocabulary_dir,
                                                  'word_to_index_%s.pkl' % self.data_type),
                                     'w'))
        pkl.dump(self.idx2word, open(os.path.join(self.vocabulary_dir,
                                                  'index_to_word_%s.pkl' % self.data_type),
                                     'w'))

        word_counts['.'] = nsents
        bias_init_vector = np.array(
            [1.0 * word_counts[w] if i > 1 else 0 for i, w in
             self.idx2word.iteritems()])
        ...
        # self.total_q = pd.DataFrame().from_csv(os.path.join(
        # dataframe_dir,'Total_desc_question.csv'), sep='\t')
        ...
        for idx, w in enumerate(answers):
            self.ans2idx[w] = idx
            self.idx2ans[idx] = w
        pkl.dump(self.ans2idx, open(os.path.join(self.vocabulary_dir,
                                                 'ans_to_index_%s.pkl' % self.data_type),
                                    'w'))
        pkl.dump(self.idx2ans, open(os.path.join(self.vocabulary_dir,
                                                 'index_to_ans_%s.pkl' % self.data_type),
                                    'w'))
        ...
        vocab = pkl.dump(glove_matrix, open(os.path.join(self.vocabulary_dir,
                                                         'vocab_embedding_%s.pkl' % self.data_type),
                                            'w'))
        self.word_matrix = glove_matrix

    def load_word_vocabulary(self):

        word_matrix_path = os.path.join(self.vocabulary_dir,
                                        'vocab_embedding_%s.pkl' % self.data_type)

        word2idx_path = os.path.join(self.vocabulary_dir,
                                     'word_to_index_%s.pkl' % self.data_type)
        idx2word_path = os.path.join(self.vocabulary_dir,
                                     'index_to_word_%s.pkl' % self.data_type)
        ans2idx_path = os.path.join(self.vocabulary_dir,
                                    'ans_to_index_%s.pkl' % self.data_type)
        idx2ans_path = os.path.join(self.vocabulary_dir,
                                    'index_to_ans_%s.pkl' % self.data_type)

        if not (os.path.exists(word_matrix_path) and os.path.exists(
                word2idx_path) and os.path.exists(idx2word_path) and
    ...
    def share_word_vocabulary_from(self, dataset):
        ...
        assert (isinstance(dataset.idx2word, dict) or isinstance(
            dataset.idx2word, list)) \
    ...
    def get_all_captions(self):
        ...
        qa_data_df = pd.DataFrame().from_csv(
            os.path.join(self.qa_dir, self.type_dict[self.data_type]), sep='\t')
    ...
    def get_video_mask(self, video_feature):
        ...
        return data_util.fill_mask(self.max_length, video_length,
                                   zero_location='LEFT')

    def get_sentence_mask(self, sentence):
        sent_length = len(sentence)
        return data_util.fill_mask(self.max_length, sent_length,
                                   zero_location='RIGHT')
    ...
    def get_Trans_result(self, chunk):
        ...
        batch_candidates = np.zeros([batch_size, 5, self.max_length],
                                    dtype=np.uint32)
        batch_candidates_right = np.zeros([batch_size, 5, self.max_length],
                                          dtype=np.uint32)
        batch_answer = np.zeros([batch_size], dtype=np.uint32)

        batch_video_mask = np.zeros([batch_size, self.max_length],
                                    dtype=np.uint32)
        batch_candidates_mask = np.zeros([batch_size, 5, self.max_length],
                                         dtype=np.uint32)
        ...
        for k in range(batch_size):
            ...
            candidates_matrix_right = self.get_Trans_matrix(candidates,
                                                            is_left=False)
            ...
            batch_debug_sent[k] = self.data_df.loc[
                key, 'a' + str(int(answer + 1))]
    ...
    def split_dataset(self, ratio=0.1):
        ...
        data_split = DatasetTGIF(args=args,
                                 mode=self.mode,
