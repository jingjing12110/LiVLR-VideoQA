...
class KnowITDataset(Dataset):
    def __init__(self, opt, mode='train'):
        super(KnowITDataset, self).__init__()
        self.opt = opt
        self.mode = mode
        self.feat_dir = os.path.join(self.opt.data_dir, 'feats/')

        self.ann_dir = "data/knowit"
        self.anns = pd.read_csv(os.path.join(
            self.ann_dir, f'knowit_data_{mode}.csv'), delimiter='\t')
        self.targets = self.anns['idxCorrect']
        self.video_names = self.anns['scene'].unique().tolist()

        # reason-answer bert embedding
        self.r_a_bert = load_pickle(os.path.join(
            self.feat_dir, f'language_bert_{mode}.pckl'))
    ...
    def open_h5(self):
        ...
        if self.opt.dataset_name == 'TVQA':
            self.feat_img = h5py.File(os.path.join(self.feat_dir,
                                                   'tvqa_imagenet_pool5_hq.h5'),
                                      'r')
        ...
        self.pembed_obj = h5py.File(
            os.path.join(self.feat_dir, f'{self.mode}_obj_position_embed_64.h5'),
            'r')
        self.adj_matrix = h5py.File(
            os.path.join(self.feat_dir, f'{self.mode}_bb_adj_matrix_11.h5'), 'r')
...
class TestMRSVTTDataset(KnowITDataset):
    ...
    def __getitem__(self, index):  # one QA pair
        ...
        feat_img = self.feat_img.get(items['vid_name'])[:]
        # TODO : change the method of sampling from the video clip
        frm_list = np.linspace(0, feat_img.shape[0], 32,
                               endpoint=False, dtype=np.int32)
        items['img_feat'] = feat_img[frm_list, :]  # [n_img, 2048]
...
if __name__ == '__main__':
    ...
    args.dataset_name = 'KnowIT'
    TVQADataset = KnowITDataset(args)
