# @File: counting.py
# @Github: https://github.com/Cyanogenoid/vqa-counting/blob/master/vqa-v2
# @Paper: Learning to Count Objects in Natural Images for Visual Question
# Answering
...
class Counter(nn.Module):
    """ Count the number of objects from a set of bounding boxes and a set of
    scores for each bounding box.
    This produces (self.objects + 1) number of count features.
    """
    def __init__(self, objects, already_sigmoid=False):
        super().__init__()
        self.objects = objects
        self.already_sigmoid = already_sigmoid

        self.f = nn.ModuleList([PiecewiseLin(16) for _ in range(16)])

    def forward(self, boxes, attention):
        """Forward propagation of attention weights and bounding boxes to
        produce count features. `boxes` has to be a tensor of shape (n, 4, m)
        ...
        Each value should be in [0, 1] if already_sigmoid is set to True,
        but there are no restrictions if already_sigmoid is set to False.
        This value should be close to 1
        if the corresponding bounding box is relevant
    ...
    @staticmethod
    def filter_most_important(n, boxes, attention):
...
class PiecewiseLin(nn.Module):
    def __init__(self, n):
        super().__init__()
        self.n = n
        self.weight = nn.Parameter(torch.ones(n + 1), requires_grad=True)
