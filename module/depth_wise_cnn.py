...
class DepthwiseSeparableConv(nn.Module):
    """
    Depth-wise separable convolution uses less parameters to
    generate output by convolution.
    :Examples:
    ...
    def __init__(self, in_ch, out_ch, k, dim=1, relu=True):
        ...
        if dim == 1:
            ...
            self.depthwise_conv = nn.Conv2d(in_channels=in_ch, out_channels=in_ch,
                                            kernel_size=k, groups=in_ch,
                                            padding=k // 2)
            self.pointwise_conv = nn.Conv2d(in_channels=in_ch,
                                            out_channels=out_ch,
                                            kernel_size=1, padding=0)
        else:
...
class ConvRelu(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, dim=1, stride=1,
                 padding=0, relu=True, dropout=0.1):
        """
